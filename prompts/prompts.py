...
[작성 지침]
- 각 소섹션(##)을 최소 5문장 이상, 구체적인 수치·사례·출처를 포함하여 상세히 서술하세요.
- 단순 나열이 아닌 분석적 서술로 작성하세요.
- 비교표와 SWOT은 반드시 마크다운 테이블 형식으로 작성하세요.
- 출처는 (출처 N, p.XX) 형식으로 본문에 명시하세요.
- [Reflect 피드백]이 있으면 해당 항목을 우선적으로 보강하세요.
...
REFERENCE_FORMAT_PROMPT = """다음은 보고서 작성에 사용된 원시 참조 목록입니다.
각 항목을 종류별로 분류하고 아래 인용 형식에 맞게 정리하세요.

[원시 참조 목록]
{raw_refs}

[출력 형식]
종류별로 구분하여 마크다운으로 출력하세요.
...
[작성 규칙]
- URL 경로와 도메인에서 기관명·제목·연도를 추론하세요.
- PDF 파일명([LG], [CATL], [Market PDF] 태그)은 '기관 보고서'로 분류하세요.
  - [LG] 파일 → LG에너지솔루션 발행 보고서
  - [CATL] 파일 → CATL 발행 보고서
  - [Market PDF] GlobalEVOutlook → IEA 발행 보고서
- [Web] URL은 도메인·경로를 보고 기관명과 제목을 추론하세요.
- 연도 불명 시 URL 내 연도 패턴 또는 2024~2025를 사용하고 [추정]을 붙이세요.
- 동일 자료의 페이지만 다른 중복 항목은 하나로 통합하세요.
- 해당 종류에 항목이 없으면 그 섹션을 생략하세요.
- 제목은 *이탤릭* 처리하세요.

정리된 참조 목록 (형식만 출력, 설명 불필요):"""
